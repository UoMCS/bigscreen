## @file
# This file contains the BigScreen-specific user handling.
#
...
sub _set_user_extradata {
    ...
    if($extradata -> {"uom_id"}) {
        my $set = $self -> _set_user_field($user -> {"user_id"}, 'uom_id', $extradata -> {"uom_id"});
        return undef if(!defined($set));

        $user -> {"uom_id"} = $extradata -> {"uom_id"} if($set);
    }

    if($extradata -> {"uom_uid"}) {
        my $set = $self -> _set_user_field($user -> {"user_id"}, 'uom_uid', $extradata -> {"uom_uid"});
        return undef if(!defined($set));

        $user -> {"uom_uid"} = $extradata -> {"uom_uid"} if($set);
    }

    if($extradata -> {"source"}) {
        my $set = $self -> _set_user_field($user -> {"user_id"}, 'source', $extradata -> {"source"});
        return undef if(!defined($set));

        $user -> {"source"} = $extradata -> {"source"} if($set);
    }
